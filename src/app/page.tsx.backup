...
export default function Home() {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash) {
        // First try to find screen by slug directly
        const screenBySlug = ledScreens.find(s => s.slug === hash);
        
        if (screenBySlug) {
          console.log('Found screen by slug:', screenBySlug);
          
          // Set city filter to screen's city
          setSelectedCity(screenBySlug.city);
          
          // Wait for map to be ready, then show popup
          setTimeout(() => {
            if (typeof window !== 'undefined' && window.showMapPopup) {
              window.showMapPopup(screenBySlug.id);
            }
          }, 1000);
        } else {
          ...
          if (parts.length >= 2) {
            ...
            // Find screen by slug or name
            const screen = ledScreens.find(s => 
              s.slug === screenName.toLowerCase() ||
